...
axios.defaults.headers.common['Authorization'] = localStorage.getItem('my-app-user')
  ? `Token ${JSON.parse(localStorage.getItem('my-app-user')).key}`
  : null;
...
class ProfileDetail extends Component {
  ...
  render() {
    ...
    let instruments;
    if (this.state.profile) {
      instruments = this.state.profile.instruments.map((instrument) => (
        <p key={instrument.text}>{instrument.text}</p>
      ));
    }

    const bandMembers = this.state.band_members.map((band_member) => (
      <motion.a
        whileHover={{ scale: 1.1, color: '#C3073F' }}
        whileTap={{ scale: 1 }}
        key={band_member.id}
        href={`/band/detail/${band_member.id}`}
      >
        {band_member.username}
      </motion.a>
    ));

    const bandMembersIds = this.state.band_members.map((band_member) => band_member.id);

    const followers = this.state.followers.map((follower) => (
      <motion.a
        whileHover={{ scale: 1.1, color: '#C3073F' }}
        whileTap={{ scale: 1 }}
        href={`/profile/detail/${follower.id}`}
      >
        {follower.username}
      </motion.a>
    ));
    ...
    const followings = this.state.following.map((following) => (
      <motion.a
        whileHover={{ scale: 1.1, color: '#C3073F' }}
        whileTap={{ scale: 1 }}
        href={`/profile/detail/${following.id}`}
      >
        {following.username}
      </motion.a>
    ));

    const followerIds = this.state.followers.map((follower) => follower.id);
      ...
      <React.Fragment>
        ...
        >
          <div className='profile-head'>
            {this.state.profile && (
              <img className='mt-5' src={this.state.profile.avatar} alt='profile' />
            )}
